...
<Callout type="info">**Update, September 2026.** PulsHealth has since become an open-source, self-hosted project. The iOS app syncs Apple Health, read-only, to a server you run yourself; the server stack, the sync protocol and a read-only MCP server for AI assistants are all in [one Apache-2.0 repository](https://github.com/PulsHealth/pulshealth). There is no hosted database, and the AI Health Agent and PrivacyProtect layer described below were not built. The [iOS app](/app) and [self-hosted server](/sync) pages describe what exists today.</Callout>

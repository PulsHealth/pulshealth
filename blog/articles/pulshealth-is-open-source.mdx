...
Two other caveats. Ingest accepts a single static bearer token, so whoever holds it can upload and delete for any user on that server; per-device tokens are the next item on the roadmap. And backups ship with the stack but are an opt-in Compose profile, off until you turn them on.
